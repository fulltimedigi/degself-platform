// Pure, RN-free, Supabase-free helpers for favorites storage + the
// guest → authenticated handoff. Kept side-effect-free so the loss-safe merge
// logic can be unit-tested with the repo's node:test runner (no RN, no DB).
//
...
// slugify an id.
//
// WHY THIS IS A MOBILE-LOCAL PORT (not a shared packages/domain import):
// the semantics are intentionally identical to apps/web-v2's favorites-sync,
// but per the architecture contract (Correction 3 / ADR-0003) we do NOT extract
// shared code merely because similar code exists. The web version is coupled to
// browser localStorage timing; this version is consumed by an async
// AsyncStorage store. Only 5 tiny pure functions are duplicated, both are unit
// tested independently, and extraction into packages/domain is deferred until a
// concrete second consumer justifies the shared-build cost. Keeping parity is
// enforced by mirrored test vectors in both apps.
...
/**
 * Parse a raw stored value into a clean, de-duplicated place_id list. Any
 * malformed payload (non-JSON, non-array, wrong element types) degrades to an
...
/** Serialize a favorites list to its canonical stored form (deduped, exact case). */
export function serializeFavorites(ids: readonly string[]): string {
  return JSON.stringify(dedupePreserveCase(ids));
}
 ...
 * the values that were in that snapshot. Any id that appeared in the guest store
 * WHILE the transfer was in flight is not in the snapshot and must survive —
 * clearing the whole store would drop it.
 */
