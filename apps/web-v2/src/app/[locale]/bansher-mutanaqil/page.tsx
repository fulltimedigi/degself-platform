...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "mobile.bansher" });
  const url = locale === DEFAULT_LOCALE ? "/bansher-mutanaqil" : `/${locale}/bansher-mutanaqil`;
  return {
    title: t("metaTitle"),
...
export default async function BansherMutanaqilPage({
  ...
  const faq = tb.raw("faq") as QA[];
  const related = tb.raw("related") as Related[];
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: tb("breadcrumbSelf"), item: `${SITE}/bansher-mutanaqil` },
    ],
  };
      ...
      <div className="mx-auto w-full max-w-6xl px-6 py-8">
        ...
        <section className="rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 p-8 sm:p-12">
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <h1 className="text-3xl font-extrabold sm:text-5xl">{tb("h1")}</h1>
            <p className="max-w-2xl text-lg text-muted-foreground">{tb("heroSubtitle")}</p>
        ...
        <section className="mt-12">
          <h2 className="mb-6 text-2xl font-extrabold">{tb("servicesHeading")}</h2>
        ...
        <section className="mt-12">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {areas.map((a) => (
              <Link
                key={a.ar}
                href={`/search?q=${encodeURIComponent("بنشر " + a.ar)}`}
        ...
        {workshops.length > 0 && (
          <section className="mt-12">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-2xl font-extrabold">{tb("workshopsHeading")}</h2>
              <Link
                href="/search?q=%D8%A8%D9%86%D8%B4%D8%B1"
